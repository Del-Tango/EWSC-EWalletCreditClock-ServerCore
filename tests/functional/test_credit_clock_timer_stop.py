...
class TestEWalletSessionManageUserActionStopTimer(unittest.TestCase):
    ...
    def test_user_action_stop_clock_timer(self):
        ...
        stop = self.session_manager.session_manager_controller(
            controller='client', ctype='action', action='stop', stop='clock_timer',
            client_id=self.client_id, session_token=self.session_token
        )
        print(str(stop) + '\n')
        self.assertTrue(isinstance(stop, dict))
        self.assertEqual(len(stop.keys()), 9)
        self.assertFalse(stop.get('failed'))
        self.assertTrue(isinstance(stop.get('credit_clock'), int))
        self.assertTrue(isinstance(stop.get('pending_count'), int))
        self.assertTrue(isinstance(stop.get('pending_time'), float))
        self.assertTrue(isinstance(stop.get('start_timestamp'), float))
        self.assertTrue(isinstance(stop.get('stop_timestamp'), float))
        self.assertTrue(isinstance(stop.get('time_record'), int))
        self.assertTrue(isinstance(stop.get('record_data'), dict))
        self.assertTrue(isinstance(stop.get('minutes_spent'), float))
