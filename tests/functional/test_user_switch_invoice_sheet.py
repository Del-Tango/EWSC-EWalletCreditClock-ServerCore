...
class TestEWalletSessionManageUserActionSwitchInvoiceSheet(unittest.TestCase):
    ...
    def test_user_action_switch_invoice_sheet(self):
        print('\n[ * ]: User action SwitchInvoiceSheet')
        instruction_set = {
            'controller': 'client', 'ctype': 'action', 'action': 'switch',
            'switch': 'invoice', 'invoice': 'list', 'sheet_id': 3,
        ...
        self.assertTrue(isinstance(switch.get('invoice_sheet'), int))
