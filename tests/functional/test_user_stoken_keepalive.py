...
class TestEWalletSessionManagerUserSTokenKeepAlive(unittest.TestCase):
    ...
    def test_user_action_stoken_keep_alive_functionality(self):
        ...
        instruction_set = {
            ...
            'alive': 'stoken', 'client_id': self.client_id,
            'session_token': self.session_token,
        }
        keep_alive = self.session_manager.session_manager_controller(
            **instruction_set
        )
        print(
            '[ > ]: Instruction Set: ' + str(instruction_set) +
            '\n[ < ]: Response: ' + str(keep_alive) + '\n'
        )
        self.assertTrue(isinstance(keep_alive, dict))
        self.assertEqual(len(keep_alive.keys()), 5)
        self.assertFalse(keep_alive.get('failed'))
        self.assertTrue(isinstance(keep_alive.get('stoken'), str))
        self.assertTrue(isinstance(keep_alive.get('extended'), int))
        self.assertTrue(isinstance(keep_alive.get('time_unit'), str))
        self.assertTrue(isinstance(keep_alive.get('stoken_data'), dict))
