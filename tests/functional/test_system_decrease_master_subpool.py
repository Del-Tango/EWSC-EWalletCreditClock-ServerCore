...
class TestEWalletSessionManagerSystemDecreaseMasterSubpool(unittest.TestCase):
...
#   @pysnooper.snoop()
    ...
    def test_system_action_decrease_master_subordonate_account_pool_size(self):
        print('\n[ * ]: System action DecreaseMasterSubpool')
        instruction_set = {
            'controller': 'system', 'ctype': 'action', 'action': 'decrease',
            'decrease': 'master', 'master': 'subpool',
            'master_id': self.master['account_data']['id'],
            'decrease_by': 3,
        }
        decrease = self.session_manager.session_manager_controller(
            **instruction_set
        )
        print(
            '[ > ]: Instruction Set: ' + str(instruction_set) +
            '\n[ < ]: Response: ' + str(decrease) + '\n'
        )
        self.assertTrue(isinstance(decrease, dict))
        self.assertEqual(len(decrease.keys()), 5)
        self.assertFalse(decrease.get('failed'))
        self.assertTrue(isinstance(decrease.get('master'), int))
        self.assertTrue(isinstance(decrease.get('subpool_size'), int))
        self.assertTrue(isinstance(decrease.get('decreased_by'), int))
        self.assertTrue(isinstance(decrease.get('master_data'), dict))
