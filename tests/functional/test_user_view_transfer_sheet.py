...
class TestEWalletSessionManageUserActionViewTransferSheet(unittest.TestCase):
    ...
    def test_user_action_view_transfer_sheet(self):
        print('\n[ * ]: User action ViewTransferSheet')
        instruction_set = {
            'controller': 'client', 'ctype': 'action', 'action': 'view',
            'view': 'transfer', 'transfer': 'list', 'client_id': self.client_id,
        ...
        self.assertTrue(isinstance(view['sheet_data'].get('ewallet'), int))
        ...
        self.assertTrue(isinstance(view['sheet_data'].get('records'), dict))
