...
class TestEWalletSessionManageUserActionUnlinkCreditClock(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Create credit clock for action unlink
        print('[...]: User action CreateCreditClock')
    ...
    def test_user_action_unlink_credit_clock(self):
        print('\n[ * ]: User action UnlinkCreditClock')
        instruction_set = {
            'controller': 'client', 'ctype': 'action', 'action': 'unlink',
            'unlink': 'credit', 'credit': 'clock', 'clock_id': 3,
        ...
        self.assertTrue(isinstance(unlink.get('credit_clock'), int))
